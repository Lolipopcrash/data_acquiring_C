...
void	ft_print_file(t_user *user, FILE *file)
{
	...
	while (user)
	{
		...
		j = -1;
		while (++j < 156)
			fprintf(file, "\"%s\": %lld,", g_seats[j], user->seats[j]);
...
void	ft_calculate(t_user *user, int size, long long start[size], long long end[size], char seat[size][8])
{
	long long	time;
	int		i;
	int		j;
...
void	ft_parse_data(t_user *user, int size)
{
	...
	fclose(file);
	ft_calculate(user, size, start_time, end_time, seat);
}
...
int	main(void)
{
	FILE	*file = fopen("aaa_seats_logtime.json", "w");
	if (file == NULL)
	{
		perror("Error opening file");
		return (-1);
